...
AbstractPouchDB.prototype.get =
  ...
  function finishOpenRevs() {
    ...
    leaves.forEach(function (leaf) {
      //parse the atts_since revpos if provided
      var attsSince = null;
      if (opts.atts_since && opts.atts_since[attsIdx]) {
        attsSince = +(opts.atts_since[attsIdx].split('-')[0]);
      }
      attsIdx++;
  ...
  //validate atts_since values if provided
  if (opts.atts_since) {
    if (Array.isArray(opts.atts_since)) {
      for (var i = 0; i < opts.atts_since.length; i++) {
        var l = opts.atts_since[i];
  ...
  return this._get(id, opts, function (err, result) {
    ...
    //if attachments option is true OR atts_since array is provided
    if ((opts.attachments || opts.revpos) && doc._attachments) {
      ...
      Object.keys(attachments).forEach(function (key) {
        //ensure we have revpos.  otherwise, default to current revision index
        if (!doc._attachments[key].revpos) {
          doc._attachments[key].revpos = +((doc._rev || '1').split('-')[0]);
        }
        
        //check for filtering with atts_since(revpos)
        //if revpos <= atts_since, then don't get attachment
        if (opts.revpos && doc._attachments[key].revpos <= opts.revpos) {
          //set the stub value and loop or return
