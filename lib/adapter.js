...
AbstractPouchDB.prototype.get =
  ...
  return this._get(id, opts, function (err, result) {
    ...
    if ((opts.attachments || opts.atts_since) && doc._attachments) {
      ...
      Object.keys(attachments).forEach(function (key) {
        ...
        //parse the atts_since revpos if specified
        var pos = 0;
        if (opts.atts_since && opts.atts_since[atts_idx]) {
          pos = +(opts.atts_since[atts_idx].split('-')[0]);
        }
        
        atts_idx++;
        
        //check for filtering with atts_since pos
        //if true, then don't get attachment
        if (pos > doc._attachments[key].revpos) {
